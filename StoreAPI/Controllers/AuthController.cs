...
/// <summary>
/// Controller responsible for user authentication and permissions management
/// </summary>
...
public class AuthController(IUnitOfWork unitOfWork, IConfiguration configuration, ITokenService tokenService, IMapper mapper, ILogger<AuthController> logger) : ControllerBase
{
    ...
    /// <response code="401">Unauthorized user, password does not match</response>
    /// <response code="200">User is authenticated successfuly</response>
    [HttpPost]
    [Route("Login")]
    public async Task<IActionResult> LoginAsync([FromBody] LoginDTO loginDTO)
    {
        ...
        if (!BCrypt.Net.BCrypt.Verify(loginDTO.Password, userExist.Password))
            return Unauthorized(new Response<IActionResult>
            {
                StatusCode = StatusCodes.Status400BadRequest,
                Message = GlobalMessage.Message("The password does not match")
            });
    ...
    /// <summary>
    /// Create a new permission/role in the system (Required Admin Role)
    /// </summary>
    /// <response code="200">Permission successfully created</response>
    /// <response code="400">Invalid role</response>
    /// <response code="409">Conflict, role already exist</response>
    ...
    public async Task<IActionResult> CreateRoleAsync([FromBody] CreateRoleDTO createRoleDto)
    {
        ...
        if (roleExist is not null)
            return Conflict(new Response<IActionResult>
            {
                ...
                Message = GlobalMessage.Conflit409
            });
    ...
    /// <summary>
    /// Add a permission/role to a user (Required Admin Role)
    /// </summary>
    /// <response code="200">Permission successfully added to the user</response>
    /// <response code="404">User not found or role not found</response>
    /// <response code="409">User already has permission</response>
    [Authorize(Policy = "AdminOnly")]
    [HttpPost]
    ...
    public async Task<IActionResult> AddRoleAsync([FromRoute] string email, [FromRoute] string roleName)
    {
        ...
        clientExist.ClientRole.Add(new ClientRole
        {
            RoleId = roleExist.Id,
    ...
    /// <summary>
    /// Delete a system permission/role (Required Admin Role)
    /// </summary>
    /// <response code="200">Permission was deleted successfully</response>
